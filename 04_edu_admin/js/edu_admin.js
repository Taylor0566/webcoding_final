Object.assign(app, {
        ...
        return students.map(s => {
            ...
            return `
                <tr>
                    <td>${s.id}</td>
                    <td>${s.name}</td>
                    <td>${s.class || '<span style="color:#999;">未分配</span>'}</td>
                    <td>${hasMajor ? s.major : '<span style="color:#ff9800;">待完善</span>'}</td>
                    <td>${courseCount}</td>
                    <td>
                        <div style="display:flex; gap:6px; justify-content:center; flex-wrap:wrap;">
                            <button class="btn btn-secondary" style="padding:4px 10px; font-size:12px; white-space:nowrap;" onclick="app.viewStudentCourses('${s.id}')">课程表</button>
                            <button class="btn btn-secondary" style="padding:4px 10px; font-size:12px; white-space:nowrap;" onclick="app.editStudent('${s.id}')">编辑</button>
                            <button class="btn btn-danger" style="padding:4px 10px; font-size:12px; white-space:nowrap;" onclick="app.deleteStudent('${s.id}')">删除</button>
                        </div>
                    </td>
                </tr>
            `;
        ...
        return courses.map(c => {
            ...
            return `
                <tr>
                    ...
                    <td>
                        <div style="display:flex; gap:6px; justify-content:center; flex-wrap:wrap;">
                            <button class="btn btn-secondary" style="padding:4px 10px; font-size:12px; white-space:nowrap;" onclick="app.viewCourseStudents('${c.id}')">学生名单</button>
                            <button class="btn btn-secondary" style="padding:4px 10px; font-size:12px; white-space:nowrap;" onclick="app.editCourse('${c.id}')">编辑</button>
                        </div>
                    </td>
                </tr>
            `;
    ...
    handleAddCourse(e) {
        ...
        // 记录操作日志
        this.logOperation('创建课程', `创建课程《${newCourse.name}》(${newCourse.id})，授课教师：${teacher.name}`);
    ...
    editCourse(courseId) {
        ...
        const modalContent = `
            <form onsubmit="app.handleEditCourse(event, '${courseId}')">
                <div class="form-group">
                    <label class="form-label">课程号</label>
                    <input type="text" class="form-input" value="${course.id}" readonly style="background:#f5f5f5;">
                </div>
                <div class="form-group">
                    <label class="form-label">课程名称 *</label>
                ...
                <div class="form-group">
                    ...
                    <select id="editCourseTeacher" class="form-input" required>
                        ${teachers.map(t => `<option value="${t.id}" ${t.id === course.teacherId ? 'selected' : ''}>${t.name}（${t.id}）</option>`).join('')}
                    </select>
                </div>
                <div class="form-group">
                    <label class="form-label">学分 *</label>
                    <input type="number" id="editCourseCredit" class="form-input" value="${course.credit}" required min="0" max="10" step="0.5">
                </div>
                <div class="form-group">
                    <label class="form-label">学期 *</label>
                    <select id="editCourseSemester" class="form-input" required>
                        <option value="2024春季" ${course.semester === '2024春季' ? 'selected' : ''}>2024春季</option>
                        <option value="2024秋季" ${course.semester === '2024秋季' ? 'selected' : ''}>2024秋季</option>
                        <option value="2025春季" ${course.semester === '2025春季' ? 'selected' : ''}>2025春季</option>
                        <option value="2025秋季" ${course.semester === '2025秋季' ? 'selected' : ''}>2025秋季</option>
                    </select>
                </div>
                <div class="form-group">
                    <label class="form-label">课程简介</label>
                    <textarea id="editCourseDesc" class="form-input" rows="3">${course.desc || ''}</textarea>
                </div>
                <div class="form-group">
                    <label class="form-label">开课学院</label>
                    <input type="text" id="editCourseDept" class="form-input" value="${course.dept || ''}">
                </div>
                <button type="submit" class="btn btn-primary" style="width:100%;">保存修改</button>
            </form>
        `;
    ...
    handleEditCourse(e, courseId) {
        ...
        if (index !== -1) {
            ...
            courses[index].name = document.getElementById('editCourseName').value.trim();
            courses[index].teacherId = teacherId;
            courses[index].teacherName = teacher.name;
            courses[index].credit = parseFloat(document.getElementById('editCourseCredit').value);
            courses[index].semester = document.getElementById('editCourseSemester').value;
            courses[index].desc = document.getElementById('editCourseDesc').value.trim();
            courses[index].dept = document.getElementById('editCourseDept').value.trim();
            
            DB.set('courses', courses);
            
            // 记录操作日志
            this.logOperation('编辑课程', `修改课程《${courses[index].name}》(${courseId})信息`);
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ${courses.map(c => {
                            ...
                            return `
                                <tr style="${!hasSchedule ? 'background:#fff3cd;' : ''}">
                                    <td>${c.id}</td>
                                    <td>${c.name}</td>
                                    <td>${c.teacherName}</td>
                                    <td>${c.schedule || '<span style="color:#ff3b30;">未排课</span>'}</td>
                                    <td>${c.classroom || '<span style="color:#ff3b30;">未分配</span>'}</td>
                                    <td>${count}人</td>
                                    <td>
                                        <button class="btn btn-primary" style="padding:4px 12px; font-size:12px;" onclick="app.editCourseSchedule('${c.id}')">设置排课</button>
                                        ${hasSchedule ? `<button class="btn btn-secondary" style="padding:4px 12px; font-size:12px;" onclick="app.clearCourseSchedule('${c.id}')">清除</button>` : ''}
                                    </td>
                                </tr>
                            `;
                        }).join('')}
                        ${courses.length === 0 ? '<tr><td colspan="7" style="text-align:center; padding:40px; color:#888;">暂无已发布课程</td></tr>' : ''}
                    </tbody>
                </table>
            </div>
    ...
    editCourseSchedule(courseId) {
        ...
        const classrooms = ['A101', 'A102', 'A201', 'A202', 'B101', 'B102', 'B201', 'B202', 'C101', 'C102'];
        
        const currentSchedule = course.schedule ? course.schedule.split(' ') : ['', ''];
            ...
            <form onsubmit="app.handleSaveCourseSchedule(event, '${courseId}')">
                <div class="form-group">
                    ...
                    <div style="background:#f5f5f5; padding:12px; border-radius:4px; margin-bottom:15px;">
                        ...
                        <span style="color:#666; font-size:14px;">教师：${course.teacherName} | 学分：${course.credit}</span>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div style="display:flex; gap:10px;">
                        <select id="scheduleWeekday" class="form-input" style="flex:1;" required>
                            ...
                            ${weekdays.map(w => `<option value="${w}" ${w === currentWeekday ? 'selected' : ''}>${w}</option>`).join('')}
                        </select>
                ...
                <div class="form-group">
                    ...
                    <select id="scheduleClassroom" class="form-input" required>
                        ...
                        ${classrooms.map(c => `<option value="${c}" ${c === course.classroom ? 'selected' : ''}>${c}</option>`).join('')}
    ...
    handleSaveCourseSchedule(e, courseId) {
        ...
        if (conflict) {
            if (!confirm(`警告：该时间段的教室已被《${conflict.name}》占用。\n\n是否仍要继续安排？`)) {
                return;
            }
        }
        ...
        if (index !== -1) {
            ...
            // 记录操作日志
            this.logOperation('设置排课', `为课程《${courses[index].name}》安排：${schedule} ${classroom}`);
    ...
    batchScheduleCourses() {
        if (!confirm('智能批量排课将为所有未排课的课程自动分配时间和教室。\n\n确定要执行吗？')) return;
        
        const courses = DB.get('courses').filter(c => c.status === 'published' && !c.schedule);
        ...
        this.logOperation('批量排课', `智能批量排课，成功安排${scheduledCount}门课程`);
    ...
    renderEduAdminGrades() {
        ...
        // 学生维度异常检测
        const studentAnomalies = this.detectStudentGradeAnomalies(enrollments, users);
        ...
        const html = `
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:20px; margin-bottom:20px;">
                ...
                <div class="card" style="background:linear-gradient(135deg, #fa709a 0%, #fee140 100%); color:white; padding:20px;">
                    ...
                    <div style="font-size:36px; font-weight:bold;">${studentAnomalies.length}</div>
                </div>
            </div>
    ...
    renderGradeCourseView(gradeStats, anomalyCount) {
        ...
        const unpublishedReady = gradeStats.filter(s => 
                ...
                <div class="card" style="margin-bottom:20px; border:2px solid #0066cc; background:linear-gradient(to right, #e3f2fd, #f8fafc);">
                    ...
                    <div style="padding:20px;">
                        <p style="margin:0 0 15px 0; color:#333;">
                            有 <strong style="color:#0066cc; font-size:20px;">${unpublishedReady.length}</strong> 门课程的成绩已全部录入完成，可以发布给学生查看：
                        </p>
                        <div style="display:flex; flex-wrap:wrap; gap:10px; margin-bottom:15px;">
                            ${unpublishedReady.map(s => `
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ${gradeStats.map(s => {
                            ...
                            const canPublish = !s.gradePublished && s.gradedStudents === s.totalStudents && s.totalStudents > 0;
                            return `
                                <tr style="${rowStyle}">
                                    <td>${s.id}</td>
                                    <td>${s.name}</td>
                                    ...
                                    <td style="${parseFloat(s.excellentRate) >= 70 ? 'color:#ff9800; font-weight:bold;' : ''}">${s.excellentRate}%</td>
                                    <td style="${parseFloat(s.passRate) < 60 && s.gradedStudents > 0 ? 'color:#ff3b30; font-weight:bold;' : ''}">${s.passRate}%</td>
                                    <td>
                                        ${s.isAnomaly 
                                    ...
                                    <td>
                                        <div style="display:flex; gap:6px; justify-content:center; flex-wrap:wrap;">
                                            <button class="btn btn-secondary" style="padding:4px 10px; font-size:12px; white-space:nowrap;" onclick="app.viewCourseGradeDetail('${s.id}')">查看详情</button>
                                            ${canPublish
                        ...
                        ${gradeStats.length === 0 ? '<tr><td colspan="9" style="text-align:center; padding:40px; color:#888;">暂无课程数据</td></tr>' : ''}
                    </tbody>
                </table>
            </div>
    ...
    detectStudentGradeAnomalies(enrollments, users) {
        ...
        students.forEach(student => {
            const studentEnrollments = enrollments.filter(e => e.studentId === student.id && e.grade !== null);
            
            if (studentEnrollments.length < 2) return; // 至少需要2门课程才能比较
            
            const grades = studentEnrollments.map(e => e.grade).sort((a, b) => a - b);
            ...
            const hasSuspicious = (minGrade < 60 && avgGrade > 75) || (maxGrade >= 85 && avgGrade < 65);
    ...
    // 切换到课程维度
    showGradeCourseView() {
        const btn1 = document.querySelector('#gradeViewContent').previousElementSibling.children[0];
        const btn2 = document.querySelector('#gradeViewContent').previousElementSibling.children[1];
        ...
        document.getElementById('gradeViewContent').innerHTML = this.renderGradeCourseView(gradeStats, anomalyCount);
    },
                    ...
                    <tbody>
                        ${studentAnomalies.map(a => `
                            <tr style="background:#fff3cd;">
                                ...
                                <td style="color:#34c759; font-weight:bold;">${a.maxGrade}</td>
                                <td style="color:#ff3b30; font-weight:bold;">${a.minGrade}</td>
                                <td style="color:#ff9800; font-weight:bold;">${a.gradeRange}</td>
                                <td>${a.stdDev}</td>
                                <td><span class="tag tag-warning">⚠️ ${a.reason}</span></td>
                            </tr>
                        `).join('')}
                        ${studentAnomalies.length === 0 ? '<tr><td colspan="10" style="text-align:center; padding:40px; color:#888;">未检测到学生成绩异常<br><small style="color:#999;">（已调整检测阈值，当前数据分布较为正常）</small></td></tr>' : ''}
                    </tbody>
                </table>
            </div>
    ...
    viewCourseGradeDetail(courseId) {
        ...
        const modalContent = `
            <h4 style="margin-bottom:15px;">${course.name}（${courseId}）成绩详情</h4>
            <table class="data-table">
                <thead>
                    <tr>
                        <th>排名</th>
                    ...
                    ${students.map((s, idx) => {
                        ...
                        return `
                            <tr>
                                ...
                                <td>${s.name}</td>
                                <td>${s.class || '未分配'}</td>
                                <td><strong>${s.grade !== null ? s.grade : '未录入'}</strong></td>
                                <td>${gradeLevel}</td>
                            </tr>
                        `;
                    }).join('')}
    ...
    publishCourseGrade(courseId) {
        ...
        if (!confirm(`确定要发布《${course.name}》的成绩吗？\n\n✅ 发布后，学生将立即可在"成绩单"中查看该课程成绩。`)) return;
        ...
        if (index !== -1) {
            ...
            courses[index].gradePublishedBy = this.state.currentUser ? this.state.currentUser.id : 'admin';
            
            // 保存到数据库
            DB.set('courses', courses);
            
            // 记录操作日志
            this.logOperation('发布成绩', `发布课程《${course.name}》(${courseId})的成绩`);
    ...
    unpublishCourseGrade(courseId) {
        ...
        if (!confirm(`确定要撤回《${course.name}》的成绩发布吗？\n\n⚠️ 撤回后，学生将无法查看该课程成绩。`)) return;
        ...
        if (index !== -1) {
            ...
            // 记录操作日志
            this.logOperation('撤回成绩发布', `撤回课程《${course.name}》(${courseId})的成绩发布`);
    ...
    batchPublishGrades() {
        ...
        const readyToPublish = courses.filter(c => {
            const courseEnrollments = enrollments.filter(e => e.courseId === c.id);
            const gradedEnrollments = courseEnrollments.filter(e => e.grade !== null);
            return c.gradePublished !== true &&  // 明确检查未发布
        ...
        const courseList = readyToPublish.map(c => `  • ${c.name} (${c.id})`).join('\n');
        
        if (!confirm(`确定要批量发布以下 ${readyToPublish.length} 门课程的成绩吗？\n\n${courseList}\n\n✅ 发布后，学生将立即可以查看这些课程的成绩。`)) return;
        ...
        // 记录操作日志
        this.logOperation('批量发布成绩', `批量发布${publishedCount}门课程的成绩：${readyToPublish.map(c => c.name).join('、')}`);
