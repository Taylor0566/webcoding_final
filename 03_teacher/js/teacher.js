...
Object.assign(app, {
  ...
  renderTeacherCourseList() {
    ...
    let courses = DB.get("courses").filter(
      (c) => c.teacherId === this.state.currentUser.id
    );
    if (lowerKeyword) {
    ...
    if (tbody) {
      tbody.innerHTML =
          ...
          .map((c) => {
            ...
            return `
                    <tr>
                        ...
                        <td>${count}</td>
                        <td>
                            <button class="btn btn-secondary" onclick="app.renderTeacherGradeEntry('${
                              c.id
                            }')">录入成绩</button>
                            <button class="btn btn-secondary" onclick="app.renderTeacherMaterials('${
                              c.id
                            }')">课件</button>
                            <button class="btn btn-secondary" onclick="app.renderTeacherSubmissions('${
                              c.id
                            }')">作业</button>
                            <button class="btn btn-secondary" onclick="app.renderTeacherEditCourse('${
                              c.id
                            }')">管理</button>
                        </td>
                    </tr>
        ...
        (pageCourses.length === 0
          ? `<tr><td colspan="5" style="color:#888; padding:18px 12px;">暂无匹配课程</td></tr>`
          : "");
    }

    const summary = document.getElementById("teacherCourseSummary");
    if (summary)
      summary.textContent = `共 ${total} 门课程，第 ${page}/${totalPages} 页`;
    ...
    if (pagination) {
      pagination.innerHTML = `
                <button class="btn btn-secondary" ${
                  page <= 1 ? "disabled" : ""
                } onclick="app.goTeacherCoursePage(${page - 1})">上一页</button>
                ${pages
                  .map(
                    (p) => `
                    <button class="btn ${
                      p === page ? "btn-primary" : "btn-secondary"
                    }" onclick="app.goTeacherCoursePage(${p})">${p}</button>
                `
                  )
                  .join("")}
                <button class="btn btn-secondary" ${
                  page >= totalPages ? "disabled" : ""
                } onclick="app.goTeacherCoursePage(${page + 1})">下一页</button>
            `;
    }
  },
    ...
    if (input) {
      ...
      if (!input.dataset.bound) {
        ...
        input.addEventListener("compositionstart", (e) =>
          this.onTeacherCourseSearchCompositionStart(e)
        );
        input.addEventListener("compositionend", (e) =>
          this.onTeacherCourseSearchCompositionEnd(e, input)
        );
        input.addEventListener("input", (e) =>
          this.onTeacherCourseSearchInput(e, input)
        );
      }
    }
  ...
  renderTeacherGradeEntry(courseId) {
    ...
    const html = `
            ...
            <div class="card">
                <div class="card-header"><h3 class="card-title">成绩录入 - ${
                  course.name
                }</h3></div>
                <table class="data-table">
                          ...
                          .map((e) => {
                            ...
                            return `
                                <tr data-sid="${s.id}">
                                    <td>${s.id}</td>
                                    <td>${s.name}</td>
                                    <td><input type="number" class="form-input" style="width:80px;" value="${
                                      e.details.homework || ""
                                    }" onchange="app.calcGrade('${
                              s.id
                            }')" id="hw_${s.id}"></td>
                                    <td><input type="number" class="form-input" style="width:80px;" value="${
                                      e.details.midterm || ""
                              ...
                              e.grade || "-"
                                    ...
                                    <td><button class="btn btn-primary" onclick="app.saveSingleGrade('${courseId}', '${
                              s.id
                            }')">保存</button></td>
                                </tr>
                            `;
            ...
            <div class="card">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                          ...
                          .map(
                            ...
                            <tr>
                              ...
                              m.name || "-"
                                ...
                                <td>${m.uploadedAt || "-"}</td>
                                <td>
                                    <button class="btn btn-secondary" onclick="app.downloadTeacherMaterial('${courseId}', '${
                              m.id
                            }')">下载</button>
                                    <button class="btn btn-danger" onclick="app.deleteTeacherMaterial('${courseId}', '${
                              m.id
                            }')">删除</button>
                                </td>
                            </tr>
                        ...
                        ${
                          materials.length === 0
                            ? `<tr><td colspan="4" style="color:#888; padding:18px 12px;">暂无课件</td></tr>`
                            : ""
                        }
                    </tbody>
                </table>
  ...
  async uploadTeacherMaterial(courseId) {
    ...
    DB.log("发布课件", `课程: ${courseId}, 文件: ${file.name}`); //日志
  ...
  deleteTeacherMaterial(courseId, materialId) {
    ...
    DB.set("courses", courses);
    DB.log("删除课件", `课程: ${courseId}, 课件ID: ${materialId}`); //日志
                    ...
                    <tbody>
                        ...
                        ${
                          assignments.length === 0
                            ? `<tr><td colspan="4" style="color:#888; padding:20px;">暂无发布的作业，点击右上角发布。</td></tr>`
                            : ""
                        }
                    </tbody>
                </table>
  ...
  async handleCreateAssignment(courseId) {
    ...
    DB.log("布置作业", `课程: ${courseId}, 标题: ${title}`); //日志
  ...
  deleteAssignment(assignmentId, courseId) {
    ...
    DB.set("assignments", assignments);
    //DB.log("删除作业", `课程: ${courseId}, 作业ID: ${assignmentId}`); //日志
    ...
    DB.log(
      "删除作业",
      `课程: ${courseId}, 作业ID: ${assignmentId}, 影响提交数: ${subs.length} 条`
    ); //日志
    this.renderTeacherSubmissions(courseId);
  },
  ...
  renderAssignmentDetail(courseId, assignmentId) {
    ...
    if (!assignment) {
      alert("作业不存在");
      return this.renderTeacherSubmissions(courseId);
    }
    ...
    const html = `
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div style="font-size:13px; color:#666; margin-top:5px;">共 ${
                      subs.length
                    } 份提交</div>
                </div>
                <div style="background:#f9f9f9; padding:15px; border-radius:4px; margin-bottom:20px; font-size:14px; color:#444;">
                    <strong>作业要求：</strong><br>${
                      assignment.content || "无文字内容"
                    }
                    ${
                      assignment.attachment
                        ? `<div style="margin-top:8px;"><a href="javascript:;" onclick="app.downloadDataUrl('${assignment.attachment.dataUrl}', '${assignment.attachment.name}')" style="color:#0066cc;">📎 下载附件: ${assignment.attachment.name}</a></div>`
                        : ""
                    }
                </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ${subs
                          .map((s) => {
                            const u = users.find(
                              (x) => x && x.id === s.studentId
                            );
                            return `
                                <tr>
                                    <td>${s.studentId}</td>
                                    <td>${u ? u.name : "-"}</td>
                                    <td><a href="javascript:;" onclick="app.downloadTeacherSubmission('${
                                      s.id
                              ...
                              s.fileName
                                    ...
                                    <td>${s.uploadedAt}</td>
                                    <td>
                                        <input type="number" id="score_${
                                          s.id
                                        }" value="${
                              s.score || ""
                            }" style="width:60px; padding:4px; border:1px solid #ddd; border-radius:4px;" placeholder="0-100">
                                    </td>
                                    <td>
                                        <button class="btn btn-primary btn-sm" onclick="app.saveSubmissionScore('${
                                          s.id
                                        }')">保存评分</button>
                                    </td>
                                </tr>
                        ...
                        ${
                          subs.length === 0
                            ? `<tr><td colspan="6" style="color:#888; padding:20px;">暂无学生提交</td></tr>`
                            : ""
                        }
                    </tbody>
                </table>
  ...
  saveSubmissionScore(submissionId) {
    ...
    if (idx !== -1) {
      ...
      DB.set("submissions", subs);
      DB.log("批改作业", `提交ID: ${submissionId}, 评分: ${score}`); //日志
  ...
  saveSingleGrade(courseId, studentId) {
    const hw =
      parseFloat(document.getElementById(`hw_${studentId}`).value) || 0;
    const mid =
      parseFloat(document.getElementById(`mid_${studentId}`).value) || 0;
    const fin =
      parseFloat(document.getElementById(`fin_${studentId}`).value) || 0;
    const total = Math.round(hw * 0.3 + mid * 0.3 + fin * 0.4);
    ...
    if (idx !== -1) {
      ...
      DB.log(
        "录入成绩",
        `学生: ${studentId}, 课程: ${courseId}, 总评: ${total}`
      ); //日志
  ...
  handleCreateCourse(e) {
    ...
    DB.set("logs", logs);
    DB.log("创建课程", `课程ID: ${new_id}, 名称: ${new_name}`); //日志
  ...
  renderTeacherEditCourse(courseId) {
    ...
    const html = `
            <button class="btn btn-secondary" onclick="app.renderTeacherDashboard()" style="margin-bottom:20px;">&larr; 返回</button>
            <div class="card">
                <div class="card-header"><h3 class="card-title">编辑课程 - ${
                  course.name
                }</h3></div>
                <form onsubmit="app.handleUpdateCourse(event, '${courseId}')">
                    <div class="form-group">
                        <label class="form-label">课程简介</label>
                        <textarea id="edit_desc" class="form-input" rows="3">${
                          course.desc || ""
                        }</textarea>
                    </div>
                    <div class="form-group">
                        <label class="form-label">上课时间</label>
                        <input type="text" id="edit_schedule" class="form-input" value="${
                          course.schedule || ""
                        }">
                    </div>
                    <div class="form-group">
                        <label class="form-label">教室</label>
                        <input type="text" id="edit_classroom" class="form-input" value="${
                          course.classroom || ""
                        }">
                    </div>
  ...
  handleUpdateCourse(e, courseId) {
    ...
    if (idx !== -1) {
      ...
      DB.set("courses", courses);
      DB.log("更新课程", `课程ID: ${courseId}, 更新字段: 描述/时间/教室`); //日志
