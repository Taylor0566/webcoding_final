const app = {
  ...
  async handleChangePassword(e) {
    ...
    if (result.success) {
      ...
      this.updateNav();
      this.showToast(
        `欢迎回来，${updatedUser.name} (${this.getRoleName(updatedUser.role)})`
      );
      this.renderDashboard();
    } else {
  ...
  renderHome() {
    ...
    const credits = [...new Set(publishedCourses.map((c) => c.credit))].sort(
      (a, b) => a - b
    );
    ...
    container.innerHTML = `
            <div class="hero-section">
                ...
                ${
                  !this.state.currentUser
                    ? `<button class="btn btn-primary" onclick="app.showLoginModal()" style="padding: 12px 30px; font-size: 18px;">立即登录</button>`
                    : ""
                }
            </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; margin-bottom: 20px; padding: 0 10px;">
                <div class="card" style="text-align: center; padding: 25px; margin-bottom: 0; display: flex; flex-direction: column; align-items: center; justify-content: center;">
                    <div style="font-size: 36px; font-weight: 700; color: #0066cc; margin-bottom: 5px; line-height: 1.2;">${
                      stats.courses
                    }</div>
                    <div style="color: #86868b; font-size: 14px; font-weight: 500;">开设课程</div>
                </div>
                <div class="card" style="text-align: center; padding: 25px; margin-bottom: 0; display: flex; flex-direction: column; align-items: center; justify-content: center;">
                    <div style="font-size: 36px; font-weight: 700; color: #34c759; margin-bottom: 5px; line-height: 1.2;">${
                      stats.users
                    }</div>
                    <div style="color: #86868b; font-size: 14px; font-weight: 500;">师生总数</div>
                </div>
                <div class="card" style="text-align: center; padding: 25px; margin-bottom: 0; display: flex; flex-direction: column; align-items: center; justify-content: center;">
                    <div style="font-size: 36px; font-weight: 700; color: #ff9500; margin-bottom: 5px; line-height: 1.2;">${
                      stats.enrollments
                    }</div>
  ...
  sendChatMessage(msg) {
    ...
    setTimeout(() => {
      ...
      if (msg.includes("密码"))
        reply =
          "您可以在登录页面点击“忘记密码”进行重置。重置需要验证您的注册邮箱。";
      else if (msg.includes("选课"))
        reply = "2024秋季学期选课将于9月1日正式开始，请留意系统公告。";
      else if (msg.includes("管理员"))
        reply = "管理员联系邮箱：support@edu.admin，或致电 0755-88888888。";
      else if (msg.includes("你好") || msg.includes("您好"))
        reply = "您好！有什么可以帮您的吗？";
  ...
  renderCourseCards(courses) {
    if (courses.length === 0)
      return '<div style="padding:20px; text-align:center; color:#888;">暂无课程</div>';
                    ...
                    ${courses
                      .map(
                        ...
                        <div style="border: 1px solid #eee; border-radius: 12px; padding: 20px; background: #fafafa; height: 100%; box-sizing: border-box;">
                            <h4 style="font-size:18px; margin-bottom:8px;">${
                              c.name
                            }</h4>
                            ...
                            <div style="font-size:12px; color:#555; margin-bottom:8px; padding: 4px 8px; background: #eef2f5; border-radius: 4px; display: inline-block;">${
                              c.dept
                            }</div>
                            <p style="font-size:13px; color:#888; margin-bottom:15px; height: 40px; overflow:hidden;">${
                              c.desc
                            }</p>
                            <div style="font-size:12px; color:#555; margin-bottom:5px;">${
                              c.schedule
                            } | ${c.classroom}</div>
                            <div style="font-size:12px; color:#666; font-style: italic;">要求: ${
                              c.requirements || "无"
                            }</div>
                        </div>
                    `
  ...
  filterPublicCourses() {
    ...
    document.getElementById("publicCourseList").innerHTML =
